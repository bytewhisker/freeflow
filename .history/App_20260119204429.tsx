...
const SidebarLink = ({ to, icon: Icon, label, active }: any) => (
  <Link
    ...
    className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${active
      ? 'bg-gray-200 text-black shadow-lg shadow-black-200'
            ...
            <>
              <aside className="w-64 bg-[#FAF9F7] border-r border-slate-200 flex flex-col sticky top-0 h-screen no-print">
                ...
                <div className="mt-auto p-6 border-t border-slate-100">
                  ...
                  <div className="flex items-center gap-3 mb-6 px-4 py-2 bg-white/50 rounded-2xl border border-slate-100/50">
                    <div className="w-10 h-10 shrink-0 rounded-full bg-blue-600 border-2 border-white flex items-center justify-center font-black text-white shadow-sm overflow-hidden">
                      {state.settings.profile?.avatarUrl ? (
                        <img src={state.settings.profile.avatarUrl} alt="Me" className="w-full h-full object-cover" />
                      ) : (
                        state.settings.profile?.name?.[0] || session.user.email?.[0]
                      )}
                    </div>
                    <div className="overflow-hidden">
                      <p className="text-sm font-black text-slate-900 truncate">
                        {state.settings.profile?.name || session.user.email?.split('@')[0]}
                      </p>
                      <p className="text-[10px] font-bold text-blue-600 uppercase  ">
                        {state.settings.profile?.title || 'Pro Member'}
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => supabase?.auth.signOut()}
                    className="flex items-center gap-3 px-4 py-2.5 w-full bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white rounded-xl text-[10px] font-black uppercase   transition-all group"
                  >
